...
Assignment #6: A Computer Pong Player
|#
...
;;------------------------------------------------------------------------------
;;Problem 1: Pong AI (Client+Server Included)
#|
...
(define AI-ACC 20) ;;pixels
;; interp. how far vertically from the center of the AI paddle should the ball
;; be for the AI-paddle to move?
...
;; -(list 'ai BallMsg PaddleMsg)
;; interp. a message sent by a computer player when the computer moves the
;; paddle.
...
;; A WaitingAI is a (new waiting-ai%)
;; interp. a computer player waiting to start the game. Once the server sends 
;; a message to start (along with what side the player will be), this will 
;; become an AIWorld and the game will begin

;; An AIWorld is one of:
;; -(new ai-left-world%), AILeftWorld
;; -(new ai-right-world%), AIRightWorld
;; interp. depending on what the server tells the WaitingAI it will be,
;; the AI will exist as a ai-left-world% (in which the computer controls the 
;; left paddle) or a ai-right-world% (in which it controls the right paddle)
...
(define-class server%
  ...
  ;; On receiving an AIMessage, the server will send multiple mails to handle
  ;; the BallMsg and PaddleMsg contained within.
  ...
  (check-expect (server2 . on-msg iworld3 msg5)
                (make-bundle 
                 (new server% iworld2 iworld3 19) 
                 (list (make-mail iworld3
                                  '(left paddle move (160) (175 down))))
                 '()))
  
  (define (on-msg iw msg)
    (cond [(symbol=? (first msg) 'ai)
           (local ((define ball-msg (second msg))
                   (define paddle-msg (third msg)))
             (if (and (this . good-ball? ball-msg) 
                      (this . good-paddle? paddle-msg))
                 (if (and (<= (this . timer) 0) 
                          (symbol=? (third ball-msg) 'move))
                     (make-bundle (new server% (this . left) 
                                  ...
                                  (list (make-mail (this . left) 
                                        ...
                                        (make-mail (this . right) 
                                                   (append '(sync) 
                                                           (rest ball-msg)))
                                        (if (symbol=? (first paddle-msg) 'left)
                                            (make-mail (this . right) 
                                                       paddle-msg)
                                            (make-mail (this . left) 
                                                       paddle-msg)))
                                  '())
                     (this . subtract-timer . on-msg iw paddle-msg))
                 (this . cheat-detected msg)))]
          [(symbol=? (second msg) 'ball)
             (if  (this . good-ball? msg)
  ...
  ;; good-ball? : BallMessage -> Boolean
  ;; is the ball information contained in this message correct for the rules of
  ;; the game? (did the player who sent this not cheat?)
  (check-expect (server1 . good-ball? msg1) true)
  (check-expect (server1 . good-ball? msg3) true)
  (check-expect (server2 . good-ball? 
                         '(left ball wait (0 0 left 0 70 (0 0))
                    (99 99 left 0 69 (0 0) 250 250))) 
                false)
  
  (define (good-ball? msg)
    (local ((define props (fourth msg))
      ...
      (equal? actual-list supposed-list)))
  
  ;;good-paddle? : PaddleMessage -> Boolean
  ;; is the paddle information contained in this message correct for the rules 
  ;; of the game? (did the player who sent this not cheat?)
  (check-expect (server1 . good-paddle? msg2) true)
  (check-expect (server2 . good-paddle? msg3) true)
  (check-expect (server2 . good-paddle? 
                         '(right ball move (296 250 right 0 0 (0 0)) 
                     (666 666 right 0 0 (0 0) 250 250)))
                false)
  
  (define (good-paddle? msg)
    (local ((define old-coord (first (fourth msg)))
                   (define new-coord (first (fifth msg))))
      (<= (abs (- new-coord old-coord)) DELTA))))
...
(define-class ball%
  ...
  (check-expect (ball1 . next 0 0) 
                (new waiting-ball% 0 (/ HEIGHT 2) 'left 0 100 '(0 1)))
  ...
  (check-expect ((new ball% -10 250 'left 0 0 '(1 1)) . score-ball)
                (new waiting-ball% 0 (/ HEIGHT 2) 'left 0 100 '(1 2)))
  
  (define (score-ball)
    (cond [(<= (this . x) 0)
           (new waiting-ball% 0 (/ HEIGHT 2) 'left 0 100
                (list (first (this . score))
                      (add1 (second (this . score)))))]
          [(>= (this . x) WIDTH)
           (new waiting-ball% 0 (/ HEIGHT 2) 'right 0 100
...
(define-class waiting-player%
  ...
  ;; AIWorld depending on the message
...
(define-class left-world%
  ...
  (check-expect (lworld1 . on-tick)
                (make-package 
                 ...
                 `(left ball move (0 0 left 0 0 (0 0)) 
                        (0 ,(/ HEIGHT 2) left 0 100 (0 1) 0 200))))
  ...
  (define (on-tick)
    (make-package (new left-world% (this . left-player) (this . right-player)
                       (this . ball . next (this . left-player . y)
                             (this . right-player . y)))
                  (this . ball . cheating-check 'left (this . left-player . y)
                        (this . right-player . y))))
...
(define-class right-world%
  ...
  (define (on-tick)
    (make-package (new right-world% (this . left-player) (this . right-player)
                       (this . ball . next (this . left-player . y)
                             (this . right-player . y)))
                  (this . ball . cheating-check 'right (this . left-player . y)
                        (this . right-player . y))))
...
(define-class waiting-ai%
  (fields)
  
  ;; register : -> IP
  ;; the IP address of the server that this ai-player connects to
  (define (register) IP-ADDRESS)
  ...
  ;; to join.
  (define (to-draw)
    (place-image (text "CONNECTING/WAITING FOR OTHER PLAYER TO JOIN..." 
                       16 'blue)
                 (/ WIDTH 2)
                 (/ HEIGHT 2)
                 MT))
  ...
  (check-expect (wai1 . on-receive '(start left))
                (new ai-left-world% (new player% 250) 
                     (new player% 250) 
                     (new waiting-ball% (/ WIDTH 2) (/ HEIGHT 2)
                          'left 0 100 '(0 0))))
  (check-expect (wai1 . on-receive '(start right))
                (new ai-right-world% (new player% 250) 
                     (new player% 250) 
                     (new waiting-ball% (/ WIDTH 2) (/ HEIGHT 2)
                          'left 0 100 '(0 0))))
  ...
  (define (on-receive msg)
    (if (symbol=? (first msg) 'start)
        (cond [(symbol=? (second msg) 'left)
               (new ai-left-world% (new player% (/ HEIGHT 2)) 
                    (new player% (/ HEIGHT 2))
                    (new waiting-ball% (/ WIDTH 2) 
                         (/ HEIGHT 2) 'left 0 100 '(0 0)))]
              [(symbol=? (second msg) 'right)
               (new ai-right-world% (new player% (/ HEIGHT 2)) 
                    (new player% (/ HEIGHT 2))
                    (new waiting-ball% (/ WIDTH 2) 
                         (/ HEIGHT 2) 'left 0 100 '(0 0)))])
        (new bad-world% msg))))

(define wai1 (new waiting-ai%))

;;------------------------------------------------------------------------------
;;AIWorld Class

(define-class ai-world%
  (super world%)
  (fields)
  
  ;; create-world : Direction LeftPlayer RightPlayer Ball -> AIWorld
  ;; based on the given direction, creates a AILeftWorld or AIRightWorld with
  ;; the given fields.
  (check-expect (ail-client1 . create-world 'left play2 play3 ball3)
                (new ai-left-world% play2 play3 ball3))
  (check-expect (air-client1 . create-world 'right play1 play2 ball4)
                (new ai-right-world% play1 play2 ball4))
  
  (define (create-world side lp rp b)
    (if (symbol=? 'left side)
        (new ai-left-world% lp rp b)
        (new ai-right-world% lp rp b)))
  
  ;; paddle-move? : Direction -> Boolean
  ;; should the paddle on the given side move in order to hit the ball?
  (check-expect (ail-client1 . paddle-move? 'left) true)
  (check-expect (ail-client1 . paddle-move? 'right) false)
  
  (define (paddle-move? side)
    (local ((define player (if (symbol=? side 'left)
                               (this . left-player)
                               (this . right-player)))
            (define ball-on-side? (if (symbol=? side 'left)
                                      (< (this . ball . x)
                                         (/ WIDTH 2))
                                      (> (this . ball . x) 
                                         (/ WIDTH 2))))
            (define 10-pad (* 0.1 PLAY-HEIGHT))
            (define will-ball-not-move? (and (<= (- (player . y) 10-pad)
                                            (this . ball . y)
                                            (+ (player . y) 10-pad))
                                             (zero? (this . ball . dy)))))
        (and ball-on-side? 
  ...
  ;; ai-ball-move : Direction -> AIWorld
  ;; creates a AIWorld for the given side in which the Ball has moved a
  ;; single tick (moved a certain amount / bounced)
  (check-expect (ail-client1 . ai-ball-move 'left)
                (new ai-left-world% (new player% 0) (new player% 200) 
                     (new waiting-ball% 0 250 'left 0 100 '(0 1))))
  (check-expect (ail-client1 . ai-ball-move 'right)
                (new ai-right-world% (new player% 0) (new player% 200)
                     (new waiting-ball% 0 250 'left 0 100 '(0 1))))
  
  (define (ai-ball-move side)
    (this . create-world side (this . left-player) 
          (this . right-player)
          (this . ball . next (this . left-player . y)
                (this . right-player . y)))))
...
(define-class ai-left-world%
  ...
  ;; also sends messages to the server to ensure that this player is not
  ;; cheating in any way
  ;; -lastly, based on the position of the ball, this will move the ai's paddle
  ;; so they will hit the ball
  ;; NOTE: Many tests would not work for 'on-tick' so there is just one.
  ;; Tests would fail even though the expected and actual values were identical.
  (check-expect (ail-client2 . on-tick)
                (make-package (new ai-left-world% (new player% 0) 
                                   (new player% 200) 
                                   (new ball% 255 250 'right 0 0 '(1 1))) 
                              '(left ball move (250 250 right 0 0 (1 1)) 
                                     (255 250 right 0 0 (1 1) 0 200))))
  
  (define (on-tick)
    (local ((define direc 'left)
            (define new-ball-world (ai-ball-move direc))
            (define ke (this . key-generator)))
    (if (paddle-move? direc)
        (make-package (new-ball-world . ai-paddle-move)
                  (list 'ai (this . ball . cheating-check direc 
                                  (this . left-player . y)
                        (this . right-player . y))
                        (this . left-player . cheating-check direc
                            (this . key-press ke . left-player . y) ke)))
    (make-package new-ball-world
                  (this . ball . cheating-check direc (this . left-player . y)
                        (this . right-player . y))))))
  
  ;; ai-paddle-move : -> AILeftWorld
  ;; depending on the position of the Ball, moves the Paddle up or down so
  ;; it will bounce the ball back
  (check-expect (ail-client1 . ai-paddle-move)
                (new ai-left-world% (new player% 0) (new player% 200) 
                     (new ball% 0 0 'left 0 0 '(0 0))))
  (check-expect (ail-client2 . ai-paddle-move)
                (new ai-left-world% (new player% 15) (new player% 200) 
                     (new ball% 250 250 'right 0 0 '(1 1))))
  
  (define (ai-paddle-move)
    (local ((define player (if (string=? "w" (this . key-generator))
                               (this . left-player . up)
                               (this . left-player . down))))
    (new ai-left-world% player (this . right-player) (this . ball))))
  
  ;; key-generator : -> KeyEvent
  ;; generates the appropriate KeyEvent such that the paddle for this world
  ;; will move to reflect the ball.
  (check-expect (ail-client1 . key-generator) "w")
  (check-expect (ail-client2 . key-generator) "s")
  
  (define (key-generator)
    (if (< (this . left-player . y) (this . ball . y))
                               "s"
                               "w"))
  
  ;; on-receive : PlayerWorld Sexpr -> PlayerWorld
  ;; receives messages about enemy movement and cheating
  (check-expect (ail-client2 . on-receive '(right paddle move (160) (175 "up")))
                (new ai-left-world% (new player% 0) 
                     (new player% 185) 
                     (new ball% 250 250 'right 0 0 '(1 1))))
  (check-expect (ail-client2 . on-receive '(right paddle move (160) (175 "down")))
                (new ai-left-world% (new player% 0) 
                     (new player% 215)
                     (new ball% 250 250 'right 0 0 '(1 1))))
  (check-expect (ail-client2 . on-receive '(cheat)) (new bad-world% '(cheat)))
  (check-expect (ail-client2 . on-receive '(badmessage)) 
                (new bad-world% '(badmessage)))
  ...
  (define (on-receive msg)
    (cond [(symbol=? (first msg) 'right)
           (cond [(symbol=? (second msg) 'paddle)
                  (local ((define world 
                            (this . key-press (second (fifth msg)))))
                    (new ai-left-world% (world . left-player) 
                         (world . right-player)
                         (world . ball)))]
                 [else (new bad-world% msg)])]
          [(symbol=? (first msg) 'sync)
           (new ai-left-world% (this . left-player) (this . right-player)
                (this . ball . msg->ball msg))]
          [else (new bad-world% msg)])))

(define ail-client1 (new ai-left-world% play1 play2 ball1))
(define ail-client2 (new ai-left-world% play1 play2 ball2))
(define ail-client3 (new ai-left-world% play3 play2 ball2))

;;------------------------------------------------------------------------------
;;AIRightWorld

(define-class ai-right-world%
  (super ai-world%)
  (fields)
  ...
  ;; -lastly, based on the position of the ball, this will move the ai's paddle
  ;; so they will hit the ball
  (check-expect (air-client1 . on-tick) 
                (make-package (new ai-right-world% (new player% 0)
                                   (new player% 200) 
                                   (new waiting-ball% 0 250 'left 0 100 '(0 1))) 
                              '(right ball move (0 0 left 0 0 (0 0)) 
                                      (0 250 left 0 100 (0 1) 0 200))))
  (check-expect (air-client2 . on-tick)
                (make-package (new ai-right-world% (new player% 0) 
                                   (new player% 200) 
                                   (new ball% 255 250 'right 0 0 '(1 1))) 
                              '(right ball move (250 250 right 0 0 (1 1)) 
    ...
    (if (paddle-move? direc)
        (make-package (new-ball-world . ai-paddle-move)
                  (list 'ai (this . ball . cheating-check direc 
                                  (this . left-player . y)
                        (this . right-player . y))
                        (this . right-player . cheating-check direc
                            (this . key-press ke . right-player . y) ke)))
    (make-package new-ball-world
                  (this . ball . cheating-check direc (this . left-player . y)
                        (this . right-player . y))))))
  ...
  ;; depending on the position of the Ball, moves the Paddle up or down so
  ;; it will bounce the ball back
  (check-expect (air-client1 . ai-paddle-move)
                (new ai-right-world% (new player% 0) (new player% 185) 
                     (new ball% 0 0 'left 0 0 '(0 0))))
  (check-expect (air-client2 . ai-paddle-move)
                (new ai-right-world% (new player% 0) (new player% 215) 
                     (new ball% 250 250 'right 0 0 '(1 1))))
  
  (define (ai-paddle-move)
    (local ((define player (if (string=? "up" (this . key-generator))
                               (this . right-player . up)
                               (this . right-player . down))))
    (new ai-right-world% (this . left-player) player (this . ball))))
  
  ;; key-generator : -> KeyEvent
  ;; generates the appropriate KeyEvent such that the paddle for this world
  ;; will move to reflect the ball.
  (check-expect (air-client1 . key-generator) "up")
  (check-expect (air-client2 . key-generator) "down")
  (check-expect (air-client3 . key-generator) "down")
  
  (define (key-generator)
    (if (< (this . right-player . y) (this . ball . y))
                               "down"
                               "up"))
  
  ;; on-receive : PlayerWorld Sexpr -> PlayerWorld
  ;; receives messages about enemy movement and cheating
  (check-expect (air-client1 . on-receive '(left paddle move (160) (175 "up")))
                (new ai-right-world% (new player% 0) (new player% 185) 
                     (new ball% 0 0 'left 0 0 '(0 0))))
  (check-expect (air-client1 . on-receive '(left paddle move (160) (175 "down")))
                 (new ai-right-world% (new player% 0) (new player% 215) 
                      (new ball% 0 0 'left 0 0 '(0 0))))
  (check-expect (air-client1 . on-receive '(cheat)) (new bad-world% '(cheat)))
  (check-expect (air-client1 . on-receive '(badmessage)) 
                (new bad-world% '(badmessage)))
  ...
  (define (on-receive msg)
    (cond [(symbol=? (first msg) 'left)
           (cond [(symbol=? (second msg) 'paddle)
                  (local ((define world 
                            (this . key-press (second (fifth msg)))))
                    (new ai-right-world% (world . left-player) 
                         (world . right-player)
                         (world . ball)))]
                 [else (new bad-world% msg)])]
          [(symbol=? (first msg) 'sync)
           (new ai-right-world% (this . left-player) (this . right-player)
                (this . ball . msg->ball msg))]
          [else (new bad-world% msg)])))

(define air-client1 (new ai-right-world% play1 play2 ball1))
(define air-client2 (new ai-right-world% play1 play2 ball2))
(define air-client3 (new ai-right-world% play3 play2 ball2))
...
;; ai-client : -> WaitingAI
;; creates the computer player client that will connect to a server and play
;; a game of pong against another player.
(define (ai-client)
  (big-bang (new waiting-ai%)))
...
;; pong-ai :
;; creates a game of Pong with a server, a human player, and a computer player
;; on a single computer. Used for testing/single player.
(define (pong-ai)
(launch-many-worlds (server)
                      (ai-client)
                      (client)))
...
;; pong-ai2 :
;; creates a game of Pong with a server and two computer players on a single
;; computer. The user gets to act as a spectator and does not interact.
(define (pong-ai2)
(launch-many-worlds (server)
                      (ai-client)
                      (ai-client)))
